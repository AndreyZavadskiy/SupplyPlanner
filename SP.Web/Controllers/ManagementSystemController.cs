...
namespace SP.Web.Controllers
{
    ...
    public class ManagementSystemController : Controller
    {
        ...
        private readonly string _title = "Система управления";
        private readonly string _className = "ManagementSystem";

        public ManagementSystemController(IMasterService masterService)
        {
        ...
        {
            var list = await _masterService.GetDictionaryListAsync<ManagementSystem>();
        ...
        {
            var model = await _masterService.GetDictionaryModelAsync<ManagementSystem>(id);
            if (model == null)
            {
                return NotFound();
            }
        ...
        public async Task<IActionResult> EditAsync([FromForm] DictionaryModel model)
        {
            ...
            var result = await _masterService.SaveDictionaryAsync<ManagementSystem>(model);
            if (result.Success)
            {
